...
// debugEnabled gates this package's diagnostic logging behind an
// environment variable -- off by default so it costs nothing in normal
// operation, but available for capturing exactly what the signature
// detector saw and when phases transitioned the next time an intermittent
// interop issue (like FT-11's occasional first-detection miss) needs to be
// pinned down.
...
func debugf(format string, args ...any) {
	...
	fmt.Fprintf(os.Stderr, "[transfer] "+format+"\n", args...)
}
